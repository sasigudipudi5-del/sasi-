...
int main()
{
    ...
    while (total_number_of_queries--) {
        ...
        if (type_of_query == 1) {
            /* append a book y pages to shelf x */
            int x, y;
            if (scanf("%d %d", &x, &y) != 2) break;

            /* current number of books on shelf x */
            int cur = total_number_of_books[x];

            /* enlarge the array for shelf x by one */
            int *new_row = (int*)realloc(total_number_of_pages[x], (cur + 1) * sizeof(int));
            if (!new_row) {
                fprintf(stderr, "realloc failed\n");
                /* free and exit */
                for (int ii = 0; ii < total_number_of_shelves; ++ii) free(total_number_of_pages[ii]);
                free(total_number_of_pages);
                free(total_number_of_books);
                return 1;
            }
            total_number_of_pages[x] = new_row;

            /* append the new book's page count and update count */
            total_number_of_pages[x][cur] = y;
            total_number_of_books[x] = cur + 1;
